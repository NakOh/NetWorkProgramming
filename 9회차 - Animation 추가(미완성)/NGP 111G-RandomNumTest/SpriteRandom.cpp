...
void CALLBACK _GameProc(HWND hWnd, UINT message, UINT wParam, DWORD lParam)
{
	...
	BackScreen->BltFast(320 + moveLeftRight, 240 + moveUpDown, SpriteImage, &SpriteRect, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
	...
	*/
	...
	scrollX--;
		for (int i = 0; i < 10; i++){
			dstRect.left = (rand()+scrollX) % 1280;			
			dstRect.top = 0 + rand() % 320;
			dstRect.right = dstRect.left + rand() % 50 + 10;
			dstRect.bottom = dstRect.top + rand() % 50 + 10;
			BackScreen->Blt(&dstRect, Gunship, &SpriteRect, DDBLT_WAIT | DDBLT_KEYSRC, NULL);
		}	
